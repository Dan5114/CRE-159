...
export default function Feedback({user, research, feedbacks_step9}) {
...
<></>
        ...
        <ul class="timeline timeline-vertical timeline-trimmed timeline-compact w-full mt-3">
            ...
            { feedbacks_step9.map((feedback_step9, index) => (
                              <>
    ...
    <div class="timeline-end mt-6 w-full">
      ...
      <span class="text-base-content/50 text-nowrap text-sm float-end m-2">{dayjs(feedback_step9.created_at).format("LLL")} (<time datetime="2023-01-23T13:23Z">{dayjs().from(dayjs(feedback_step9.created_at), true)} ago</time>) &nbsp;
        ...
        <p class="mb-2 text-xs mt-3">Seen by: <span class="icon-[tabler--checks] text-success align-bottom"></span></p>
            ...
            <div class="text-sm">
            <p class="text-sm">{feedback_step9.seen_by}</p>
            <p class="text-xs font-bold">Researcher</p>
            <p class="text-xs">{dayjs(feedback_step9.updated_at).format("LLL")} (<time datetime="2023-01-23T13:23Z">{dayjs().from(dayjs(feedback_step9.updated_at), true)} ago</time>)</p>
            </div>
        </div>
                    </>
                    :
